...
public class PipeManager {
    ...
    public final float gapHeight = 0.5f;
    ...
    public void update(float dt, List<Bird> birds) {
        ...
        while (it.hasNext()) {
            ...
            for (int i = 0; i < birds.size(); i++) {
                Bird b = birds.get(i);
                if (b.collidesWith(p.x, p.gapY, width, gapHeight)) b.alive = false;
                if (p.x + width/2 < b.x) {
                    if (i == 0 && !p.p1Passed) { p.p1Passed = true; b.score++; }
                    if (i == 1 && !p.p2Passed) { p.p2Passed = true; b.score++; }
                }
            }
            if (p.x < -1.5f) it.remove();
        }
    }
    ...
    private void draw(int uOff, int uSca, int uCol, float x, float y, float w, float h) {
        ...
        GL20.glUniform3f(uCol, 0.2f, 0.7f, 0.3f);
        GL11.glDrawArrays(GL11.GL_TRIANGLES, 0, 6);
    }

    public void reset() { pipes.clear(); timer = 0; }
}
